...
credential-like files and hash/member mismatches. `--verify` builds twice,
compares archive bytes, extracts into a temporary empty directory and runs the
repository, requirements-boundary, Proof Binding and Proof Engine validators
offline. The Proof Binding ledger pins authority artifacts and PR/CI receipts
but does not promote partial C13–C15 evidence.

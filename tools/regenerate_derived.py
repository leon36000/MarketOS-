...
def _execution_contract_count(root: Path) -> int:
    return len(list((root / "execution-contracts").glob("*.md"))) + len(list((root / "planning" / "phases").glob("*/EXECUTION_CONTRACT.md")))
...
def _manifest_entries(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.name in IGNORED_FILES or path.suffix == ".pyc" or any(part in IGNORED_DIRS for part in relative.parts):
            continue
        content = path.read_bytes()
        entries.append({"path": relative.as_posix(), "sha256": hashlib.sha256(content).hexdigest(), "bytes": len(content)})
    return entries
...
def build_outputs(root: Path) -> dict[Path, str]:
    ...
    manifest = {"version": "0.4.0", "files": _manifest_entries(root)}
    return {
        requirement_index_path: json.dumps(requirement_index, ensure_ascii=False, indent=2) + "\n",
        phase_index_path: json.dumps(phase_index, ensure_ascii=False, indent=2) + "\n",

#!/usr/bin/env python3
"""Build and verify a deterministic MARKET-OS design handoff archive."""
...
def validate_source(root: Path, *, require_clean: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        "c16": _run_json_validator(root, "tools/validate_c16_design.py"),
    }
...
def _read_first(commit: str) -> bytes:
    return (
        ...
        "This is a verified design and implementation handoff, not completed trading software.\n\n"
        "```yaml\n"
        ...
        "```\n\n"
        "Start in `repository/` and run:\n\n"
        "```bash\n"
        "python tools/validate_repository.py --root . --json\n"
        "python tools/validate_c16_design.py --root . --json\n"
        "```\n\n"
        "Implementation begins at node `00` in `implementation/IMPLEMENTATION_DAG.json`. "
        "All nodes are initially `NOT_STARTED`; nodes 37–39 remain policy-blocked.\n"
    ).encode("utf-8")
...
def _members(
    root: Path,
    tracked: Iterable[tuple[str, int]],
    info: dict[str, Any],
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        ...
        with tempfile.TemporaryDirectory(prefix="marketos-pack-verify-") as temp_dir:
            ...
            if run_repository_checks:
                ...
                reports["c16"] = _run_json_validator(repository_root, "tools/validate_c16_design.py")
            if not (repository_root / "implementation/IMPLEMENTATION_DAG.json").is_file():
                raise PackError("extracted repository is missing implementation DAG")
...
def build_and_verify(root: Path, output: Path, *, require_clean: bool = True) -> dict[str, Any]:
    ...
    output.with_name(output.name + ".verification.md").write_text(
        "# MARKET-OS Build Pack Verification\n\n"
        f"- Archive: `{output.name}`\n"
        f"- SHA-256: `{first['archive_sha256']}`\n"
        f"- Commit: `{first['commit']}`\n"
        f"- Tree: `{first['tree']}`\n"
        f"- Members: `{first['member_count']}`\n"
        "- Deterministic rebuild: `PASS`\n"
        "- Clean extraction and offline validators: `PASS`\n"
        "- Software implementation: `NOT_COMPLETE`\n"
        "- Profitability: `UNPROVEN`\n"

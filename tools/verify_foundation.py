...
def _attempt_guarded_event_mutation(path: Path) -> None:
...
def _store_check() -> str:
    with tempfile.TemporaryDirectory(prefix="marketos-foundation-store-") as temp_dir:
        ...
        _attempt_guarded_event_mutation(path)
        try:
...
def _risk_check(limits: RiskLimits) -> str:
...
def _paper_check(limits: RiskLimits) -> str:
...
def _replay_check(limits: RiskLimits, scenario: tuple[EventEnvelope, ...]) -> str:
    config = ReplayConfig(
        "acceptance",
        limits.currency,
...
def verify() -> dict[str, object]:
    ...
    checks = [
        _check("canonical-evidence", _canonical_check),
        _check("exact-money", _money_check),
        _check("event-order", _event_order_check),
        _check("hash-chain-tamper", _store_check),
        _check("double-entry-reversal", _ledger_check),
        _check("risk-allow-and-veto", lambda: _risk_check(limits)),
        _check("paper-execution", lambda: _paper_check(limits)),
        _check("deterministic-replay", lambda: _replay_check(limits, scenario)),
        _check("live-route-absent", _lock_check),
    ]
    return {
